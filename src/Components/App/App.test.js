...
describe("App", () => {
  ...
  it("Navigates to Add Sk8Map view", async () => {
    ...
    const navAddSk8MapBtn = screen.getAllByRole("link", {
      name: /add a sk8map/i,
    });
    userEvent.click(navAddSk8MapBtn[0]);

    const addSk8MapHeader = screen.getByRole("heading", {
      name: /click the map to add a new sk8map/i,
    });
  ...
  it("Navigates to a specific spot by url", async () => {
    ...
    const spotDetailTitle = await waitFor(() =>
      screen.getByRole("heading", { name: /old church/i })
    );
  ...
  it("shows helpful redirect text at invalid endoint", async () => {
    ...
    const redirectText1 = await waitFor(() =>
      screen.getByRole("heading", {
        name: /looks like that's not a valid url/i,
      })
    );
    const redirectText2 = screen.getByRole("heading", {
      name: /try double checking that the url is correct,/i,
    });
    const redirectText3 = screen.getByRole("heading", {
      name: /or head over to the/i,
    });
  ...
  it("be able to create a new map", async () => {
    ...
    const confirmMarker = await waitFor(() =>
      screen.getByRole("button", { name: /confirm placement/i })
    );
    expect(confirmMarker).toBeInTheDocument();
    userEvent.click(confirmMarker);

    const feature1 = await waitFor(() =>
      screen.getByRole("heading", { name: /curbs/i })
    );
